...
the reasons for doing so. Attributed delay of the kind above is the
most visible prompt, though not the commonest. An aircraft
...
airspace~\cite{prc2026}. Not all of that distance is avoidable:
...
of the rest is a cheaper route that the operator would have been willing
to fly and did not file. That such routes exist is assumed here rather
than shown.
% Cut for space 2026-09-07 (round four; the journal keeps it, and
% Section IV-A of that version returns to the assumption):
% ... and a route closed by a restriction or an unavailable conditional
% route is not one of them.
 Candidate routes are not hard to come by. What is hard is knowing
which candidate this particular operator would accept, on this day, with
this rotation to protect, and knowing it while there is still time to
file.
...
in ten neither route was caught by it
...
minority that was caught is where the choice is sharpest.
