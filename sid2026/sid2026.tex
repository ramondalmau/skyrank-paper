...
instead. This paper learns from 1.48 million such pairs, drawn from
eighteen months of European traffic, which of two routes an airline
...
fitted on the first fifteen months and evaluated once on the last three.
It identifies the route the airline chose 68.1\,\% of the time, against
55.0\,\% for the strongest rule that can be built on any single cost
indicator. Because its confidence is calibrated, a channel proposing
...
does. The archive is built
from flights that changed route, however, so the model is largely untested
on the majority that stay, and it may propose change more readily than
airlines would accept it. Even so, what an airline will accept can be read
from what it has already filed.
...
and this archive holds one of the mechanisms behind that figure. A
...
trade is worth making is the operator's judgement, and the archive shows
it being made in both directions. This work assumes, rather than
shows, that some fuel-cheaper alternatives go unfiled because nobody had
reason to look for them in time; others go unfiled because they could not
have been filed at all. Candidate routes are not hard to come by; what is hard is knowing which
...
The pairs that cost fuel lie elsewhere: the revisions that escape a
...
reported unchanged on the evaluation months. The calibrator always scores the route
...
against. The same rule scores 45.0\,\% across all held-out pairs, the
small difference being the population and not the rule.
...
the tuning month scores 67.5\,\%, so there is no sign of overfitting to
it.
...
carries structure no one indicator recovers and was by no means
self-evident.
...
deployable, and across the three test months accuracy moves between
67.5\,\% and 69.0\,\%.
...
Adding stay pairs at group weight 10 turns 34.8\,\% into 90.0\,\%, and
that is the largest correction any of these choices makes. The row
records its price, and the 34.8\,\% is the change bias of
Section~\ref{sec:method}, in that a model trained on revisions alone
prefers the alternative route in 65.2\,\% of the cases where the flight
demonstrably kept its own, and so is right on the remaining 34.8\,\%:
revisions teach it that a regulated route is one to escape, and it
carries that lesson into a population where the opposite holds. The
...
confident about, it is right 94.9\,\% of the time. Secondly, much of what the model can learn is decided before training, by
...
abandoned routes barely better than chance, which points at the geometric
input rather than the objective as the limitation. A name-independent
