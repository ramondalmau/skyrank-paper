...
## Blocking before submission: regenerate Fig. 2

`figures/fig_pipeline.png` carries nine defects, all verified by opening the
image and listed with their repairs in
`figures-src/fig_pipeline_paperbanana_chatgpt.md`. Five of them make the
figure disagree with the manuscripts, and one is severe: the bar chart of
the third stage draws the airline choosing the cheaper route on every
indicator, which is the opposite of the paper's central finding. The
captions of both manuscripts now state the three that a reader would
otherwise take as fact, which is a stopgap, not a repair.

The figure can only be regenerated by pasting the four messages of that
bundle into one ChatGPT conversation by hand, in order, and cropping the
result: image generation is not reachable from this machine (OpenRouter
returns HTTP 402, the Gemini image models 429, both checked 2026-09-06), and
the standing rule for this figure is a raster from those prompts, never a
vector redraw. The bundle's Methodology section has been corrected so that a
re-run does not reproduce the same bar chart. Until the raster is replaced,
neither paper should be submitted and the figure must not be offered as the
graphical abstract.
